...
g_ppa_df = train_df.groupby('yearmonth')['price_doc'].aggregate(np.mean).reset_index()
...
train_y_2 = []
for i in range(0, len(train_df['price_doc'])):
	mppa = g_ppa_df.loc[ymid(train_df.loc[i, 'yearmonth']), 'price_doc']
	train_y_2.append(train_df.loc[i, 'price_doc'] - mppa)

train_X_2 = train_df.drop(['index', 'id', 'timestamp', 'price_doc'], axis=1)
